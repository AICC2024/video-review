// Handler for the "📎 Comment from Annotation" button.
document.addEventListener("DOMContentLoaded", () => {
  const commentBtn = document.getElementById("screenshotAnnotation");
  if (commentBtn) {
    commentBtn.addEventListener("click", () => {
      ...
      if (!canvas) {
        alert("No visible canvas to capture.");
        return;
      }

      const imageData = canvas.toDataURL("image/png");
      console.log("📎 Screenshot captured for comment:", {
        page: currentPage,
        image: imageData
      });

      // In future: postMessage to parent window or pre-fill a comment form
    });
  }
});
/**
...
// Allow loading PDFs from cross-origin sources like S3
_app_options_js__WEBPACK_IMPORTED_MODULE_2__.AppOptions.set('sandboxExternalContent', false);
...
{
  // Override the origin check to allow cross-origin PDFs (e.g., from S3)
  var validateFileURL = function (file) {
    // ✅ Disable strict origin check for cross-domain PDFs
    return file;
  };
}
...
function webViewerLoad() {
  // Disable sandbox restriction for external content and preferences override.
  _app_options_js__WEBPACK_IMPORTED_MODULE_3__.AppOptions.set('sandboxExternalContent', false);
  _app_options_js__WEBPACK_IMPORTED_MODULE_3__.AppOptions.set('disablePreferences', true);
...
// Screenshot annotation button logic
document.addEventListener("DOMContentLoaded", () => {
  const commentBtn = document.getElementById("screenshotAnnotation");
  if (commentBtn) {
    commentBtn.addEventListener("click", () => {
